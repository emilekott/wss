...
#
# $Id: m1_export.php 951 2011-08-03 13:43:07Z vovav $
#
...
switch (m1_export_module :: getShoppingCartType()) {
	default: 
	case 'oscommerce':
            if (function_exists(tep_get_version)) {  //Oscommerce >=2.3
                $cl_box_groups[] = m1_export_all_show_box_oscommerce23($formats);
            } else {
                m1_export_all_show_box_oscommerce($formats); //Oscommerce <2.3
            }
            break;
...
/**
 * Get module links in admin box of osCommerce >= 2.3
 *
 * @param array $formats
 * @return array $cl_box_group
 */

function m1_export_all_show_box_oscommerce23($formats) {
    $cl_box_group['heading'] = M1OutputConstant('M1_ALLINONE_MAINMENU_PREFIX').'Export Tools';

    //Sales Channel Analysis link
    $cl_box_group['apps'][] = array(
        'code' => 'm1_export_stats.php',
        'title' => 'Sales Channel Analysis',
        'link' => tep_href_link('m1_export_stats.php', 'selected_box=m1_export')
    );
    ...
    foreach ($formats as $v) {
        ...
        if (file_exists($classFile)) {
            ...
            $cl_box_group['apps'][] = array(
                'code' => $linkFilename,
                'title' => $linkObject->Caption .' Export',
                'link' => tep_href_link($linkFilename)
            );
        }
    }
