...
if ($_SESSION['html_editor_preference_status']=="XINHA") {

//define URL and LANG parameters
  echo '<script type="text/javascript">' .BR;
  echo '   _editor_url = "'.DIR_WS_CATALOG . 'editors/xinha/";' .BR;
   echo '	_editor_lang = "'.strtolower($_SESSION['languages_code']).'";' .BR;
  echo '</script>' .BR;

//<!-- load the main HTMLArea files -->
  echo '<script type="text/javascript" src="' . DIR_WS_CATALOG . 'editors/xinha/XinhaCore.js"></script>' .BR;
...
echo '      HTMLArea.loadPlugin("TableOperations");' .BR;
  ...
  echo '	  HTMLArea.loadPlugin("ExtendedFileManager");' .BR;
   echo '	  HTMLArea.loadPlugin("ListType");' .BR;
   echo '	  HTMLArea.loadPlugin("CharacterMap");' .BR;
