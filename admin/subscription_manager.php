    ...
    if (zen_not_null($action)) {
      switch ($action) {
        case 'insert':
          ...
          $postal_code = zen_db_prepare_input($_POST['postal_code']);
          $birthday = zen_db_prepare_input($_POST['birthday']);

		  $arr_date = split("/", $birthday);
		  $birthday = "$arr_date[2]-$arr_date[1]-$arr_date[0]";
          ...
          if ($check_cust_email->fields['total'] > 0) {
            ...
            $db->Execute("insert into " . TABLE_SUBSCRIBERS . "
                      (email_address, email_format, confirmed, subscribed_date, first_name, last_name, birthday, postal_code)
                      values ('" . zen_db_input($email_address) . "', '" . 
                                   zen_db_input($email_format) . "', '1', now(), '".

                                   zen_db_input($first_name) . "', '".
                                   zen_db_input($last_name) . "', '".
                                   zen_db_input($birthday) . "', '".
        ...
        case 'save':
          ...
          $db->Execute("update " . TABLE_SUBSCRIBERS . "
                        set subscriber_id = '" . (int)$subscriber_id . "',
							...
							first_name = '" . zen_db_input($first_name) . "',
							last_name = '" . zen_db_input($last_name) . "',
							birthday = '" . zen_db_input($birthday) . "',
							postal_code = '" . zen_db_input($postal_code) . "'
...
<?php
  ...
  if(!empty($filter)) {
    ...
    $classes_query_raw = 
      "select subscriber_id, customers_id, email_address, email_format, confirmed, subscribed_date, first_name, last_name, birthday, postal_code from " . TABLE_SUBSCRIBERS . $filter_str .
      " order by ".$disp_order;
  ...
  if(!empty($classes)) {
    while (!$classes->EOF) {
      if ((!isset($_GET['tID']) || (isset($_GET['tID']) && ($_GET['tID'] == $classes->fields['subscriber_id']))) && !isset($tcInfo) && (substr($action, 0, 3) != 'new')) {
		...
		$arr_date = split("-", $tcInfo->birthday);
		$tcInfo->birthday_formatted = $arr_date[2]."/".$arr_date[1]."/".$arr_date[0];
      }
...
<?php
  ...
  switch ($action) {
    ...
    case 'new':
      ...
      $contents[] = array('text' => '<br />First Name' . '<br />' . zen_draw_input_field('first_name', '', zen_set_field_length(TABLE_SUBSCRIBERS, 'first_name')));
      $contents[] = array('text' => '<br />Last Name' . '<br />' . zen_draw_input_field('last_name', '', zen_set_field_length(TABLE_SUBSCRIBERS, 'last_name')));
      $contents[] = array('text' => '<br />Postal Code' . '<br />' . zen_draw_input_field('postal_code', '', zen_set_field_length(TABLE_SUBSCRIBERS, 'postal_code')));
      $contents[] = array('text' => '<br />Birthday (dd/mm/yyyy)' . '<br />' . zen_draw_input_field('birthday', '', zen_set_field_length(TABLE_SUBSCRIBERS, 'birthday')));
    ...
    case 'edit':
      ...
      $contents[] = array('text' => '<br />First Name' . '<br />' . zen_draw_input_field('first_name', $tcInfo->first_name, zen_set_field_length(TABLE_SUBSCRIBERS, 'first_name')));
      $contents[] = array('text' => '<br />Last Name' . '<br />' . zen_draw_input_field('last_name', $tcInfo->last_name, zen_set_field_length(TABLE_SUBSCRIBERS, 'last_name')));
      $contents[] = array('text' => '<br />Postal Code' . '<br />' . zen_draw_input_field('postal_code', $tcInfo->postal_code, zen_set_field_length(TABLE_SUBSCRIBERS, 'postal_code')));
      $contents[] = array('text' => '<br />Birthday (dd/mm/yyyy)' . '<br />' . zen_draw_input_field('birthday', $tcInfo->birthday_formatted, zen_set_field_length(TABLE_SUBSCRIBERS, 'birthday')));
