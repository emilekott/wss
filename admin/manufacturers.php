...
<?php
  $manufacturers_query_raw = "select manufacturers_id, manufacturers_name, manufacturers_description, manufacturers_image, date_added, last_modified from " . TABLE_MANUFACTURERS . " order by manufacturers_name";
  ...
  switch ($action) {
    ...
    case 'edit':
      ...
      if ($_SESSION['html_editor_preference_status']=='FCKEDITOR') {
                $oFCKeditor = new FCKeditor('manufacturers_description') ;
                $oFCKeditor->Value = $mInfo->manufacturers_description;
...
//        $category_inputs_string .= '<INPUT type="hidden" name="categories_description[' . $languages[$i]['id']  . ']" ' . 'value=' . "'" . zen_get_category_description($cInfo->categories_id, $languages[$i]['id']) . "'>";
        ...
        $category_inputs_string .= zen_draw_textarea_field('manufacturers_description', 'soft', '100%', '20', $mInfo->manufacturers_description);
      }
    $contents[] = array('text' => '<br />' . 'Manufacturer description' . $category_inputs_string);
