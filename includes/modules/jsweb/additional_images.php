...
$SMALL_IMAGE_WIDTH_x = 90;
$SMALL_IMAGE_HEIGHT_x = 90;
...
if ($num_images) {
  ...
  for ($i=0, $n=$num_images; $i<$n; $i++) {
    ...
    //$link = $script_link . "\n      " . $noscript_link;
    $link = '<a href="'.$products_image_large.'" rev="'.$products_image_large.'" rel="zoom-id:ancid1;" onclick="changeZooming(\''.$products_image_large.'\', \''.$base_image.'\'); return false;">'.'<img height="'.$SMALL_IMAGE_HEIGHT_x.'" width="'.$SMALL_IMAGE_WIDTH_x.'" src="'.$base_image.'" />'.'</a>';
    //      $link = $alternate_link;

    // List Box array generation:
    $list_box_contents[$row][$col] = array('params' => 'class="additionalImages centeredContent back"' . ' ' . 'style="width:' . '100' . 'px;"',
    'text' => "\n      " . $link);
    $col ++;
